...
public class ValidationItemControllerV2 {
    ...
    @InitBinder
    public void init(WebDataBinder dataBinder) { //해당 controller 호출될 때마다 WebDataBinder 생성
        log.info("init binder {}", dataBinder);
        dataBinder.addValidators(itemValidator); //메서드 호출 전 WebDataBinder에 미리 검증기를 넣어둠으로써 모든 메서드에 적용되게 해줌
    }
...
//    @PostMapping("/add")
    public String addItemV1(@ModelAttribute Item item,
                          BindingResult bindingResult, // Item 에 담기지 못할 것들을 담는 곳
                          RedirectAttributes redirectAttributes,
                          Model model) {

        //검증 로직
        if (!StringUtils.hasText(item.getItemName())) { // 이름이 없으면 오류 데이터를 기록
        ...
        if (item.getPrice() != null && item.getQuantity() != null) {
            ...
            if (resultPrice < 10000) {
                bindingResult.addError(new ObjectError("item", "가격 * 수량의 합은 10,000원 이상이아야 합니다. 현재 값은 = " + resultPrice +"원"));
            }
        }
        //검증이 실패하면 다시 입력 폼으로
...
//            bindingResult.rejectValue("itemName", "required.test"); // 이렇게 코드 사용도 가능 -> messageCodesResolver 덕분
        }
        // 위의 if 문과 동일한 기능 -> ValidationUtils 한줄 코딩 가능, 편리성 제공
...
//        itemValidator.supports(item) // 실무에서 if로 검증하면서 넘어감
